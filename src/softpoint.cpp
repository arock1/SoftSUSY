...
void errorCall() {
  ...
  ii << "[FGM params]: --beta2=<value> --beta3=<value>\n\n";
...
int main(int argc, char *argv[]) {
  ...
  if (argc == 1 || ( (strcmp(argv[1], "sugra") && 
		      strcmp(argv[1], "amsb") &&
		      strcmp(argv[1], "gmsb") && 
		      strcmp(argv[1], "FGMCaseB0") &&
		      strcmp(argv[1], "FGMCaseB1") &&
  ...
  if (strcmp(argv[1], "leshouches")) {
	...
	cout << "Make sure you install the CLN and GiNaC packages beforehand.\n";
	  ...
	  else if (starts_with(argv[i], "--beta2=")){
	...
	if (cgrav > 1.0) {
      ...
      // Todd edits 
    if (!strcmp(argv[1], "FGMCaseB0")) {
	cout << "# SOFTSUSY FGM CaseB0" << endl;
	
	flavourViolation = true;
	boundaryCondition = &FGMCaseB0bcs;
	modelIdent = "FGMCaseB0";
    printRuledOutSpectra = false;
	pars.setEnd(3);
	pars(1) = LAMBDA; pars(2) = mMess; pars(3) = cgrav;
	if (mMess < 1000.) {
	  ostringstream ii; 
	  ii << " mMess=" << mMess
	...
	r = &k; 
	    ...
	    if (!strcmp(argv[1], "FGMCaseB1")) {
	cout << "# SOFTSUSY FGM CaseB1" << endl;
	
	flavourViolation = true;
	boundaryCondition = &FGMCaseB1bcs;
	modelIdent = "FGMCaseB1";
    printRuledOutSpectra = false;
	pars.setEnd(5);
	pars(1) = LAMBDA; pars(2) = mMess; pars(3) = beta2; pars(4) = beta3; pars(5) = cgrav;
	if (mMess < 1000.) {
	  ostringstream ii; 
	  ii << " mMess=" << mMess
	     << " in FGM input (too low). The point will not yield a sensible answer\n";
	...
	r = &k; //telling it to flag the flavorful version
	    ...
	    if (!strcmp(argv[1], "FGMCaseB1Large")) {
	cout << "# SOFTSUSY FGM CaseB1Large" << endl;
	
	flavourViolation = true;
	boundaryCondition = &FGMCaseB1Largebcs;
	modelIdent = "FGMCaseB1Large";
    printRuledOutSpectra = false;
	pars.setEnd(3);
	pars(1) = LAMBDA; pars(2) = mMess; pars(3) = cgrav;
	if (mMess < 1000.) {
  ...
  r = &k; //telling it to flag the regular version
	...
	flavourViolation = true;
	boundaryCondition = &FGMCaseB1Altbcs;
	modelIdent = "FGMCaseB1Alt";
    printRuledOutSpectra = false;
	pars.setEnd(5);
	pars(1) = LAMBDA; pars(2) = mMess; pars(3) = beta2; pars(4) = beta3; pars(5) = cgrav;
	if (mMess < 1000.) {
    ...
    if (!strcmp(argv[1], "FGMCaseB2")) {
	cout << "# SOFTSUSY FGM CaseB2" << endl;
	
	flavourViolation = true;
	boundaryCondition = &FGMCaseB2bcs;
	modelIdent = "FGMCaseB2";
    printRuledOutSpectra = false;
	pars.setEnd(5);
	pars(1) = LAMBDA; pars(2) = mMess; pars(3) = beta2; pars(4) = beta3; pars(5) = cgrav;
	if (mMess < 1000.) {
	...
	if (beta2 > 1) {
	  ostringstream ii; 
	  ii << " beta2=" << beta2
	     << " in FGM input (too high). The point will not yield a sensible answer\n";
	  throw ii.str();
	}
	else if (beta2 < -1) {
	  ostringstream ii; 

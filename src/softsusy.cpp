...
void MssmSoftsusy::neutralinoCharginoMSLHA(ostream & out) {
  ...
  out << "   # ~neutralino(2)\n";
  /// Charginos printed out positive in SLHA format, since they are sorted out
  /// by the mixing matrix minus row.
...
void MssmSoftsusy::gaugeSLHAHigh(ostream & out) {
  double gp = displayGaugeCoupling(1) * sqrt(0.6);
  out << "Block gaugeHigh Q= " << displayMu() << "  # SM gauge couplings\n";
  out << "     1     " << gp << "   # g'(Q)MSSM DRbar"
      << endl;   
  out << "     2     " << displayGaugeCoupling(2) 
      << "   # g(Q)MSSM DRbar" << endl;   
...
void MssmSoftsusy::yukawasSLHAHigh(ostream & out) {
  out << "Block yuHigh Q= " << displayMu() << "  \n"
      << "  3  3     " << displayYukawaElement(YU, 3, 3) 
      << "   # Yt(Q)MSSM DRbar" << endl;
  out << "Block ydHigh Q= " << displayMu() << "  \n"
      << "  3  3     " << displayYukawaElement(YD, 3, 3) 
      << "   # Yb(Q)MSSM DRbar" << endl;
...
void MssmSoftsusy::hmixSLHAHigh(ostream & out) {
  out << "Block hmixHigh Q= " << displayMu() << 
    " # Higgs mixing parameters\n";
  out << "     1    "; printRow(out, displaySusyMu()); 
  out << "    # mu(Q)MSSM DRbar\n";
  out << "     2    "; printRow(out, displayTanb()); 
...
void MssmSoftsusy::msoftSLHAHigh(ostream & out) {
...
void MssmSoftsusy::drbarSLHAHigh(ostream & out, int numPoints, double qMax, int n) {
  /// Starting non-essential information. The following decides what scale to
  /// output the running parameters at. It depends upon what qMax is and how
  /// many points the user has requested.
  /// For qMax = 0 and 1 point (defaults), Q=MSUSY is printed out.
...
MssmSoftsusy::displayPhysUncertainty(TMSSMBoundaryCondition bc,
   ...
   // combine
   const DoubleVector total = v_scale.abs() + v_match.abs() +
      v_mt.abs() + v_alphas.abs() + v_alphaem.abs();

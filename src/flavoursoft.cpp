...
void FlavourMssmSoftsusy::yukawasSLHAHigh(ostream & out) {
  /// Find the Yukawa rotation matrices such that the super-CKM basis may be
  /// defined
  DoubleMatrix Ud(3, 3), Vd(3, 3), Uu(3, 3), Vu(3, 3);
  DoubleVector yu(3), yd(3);
  ...
  out << "Block yuHigh Q= " << displayMu() << "   # diagonal Up Yukawa matrix\n";
  int i; for (i=1; i<=3; i++) {
    out << "  " << i << "  " << i << "     " << yu(i)
	<< "    # YU_{" << i << i << "}(Q)MSSM DRbar" << endl;
  }
  
  out << "Block ydHigh Q= " << displayMu() << "   # diagonal down Yukawa matrix\n";
  for (i=1; i<=3; i++) {
    out << "  " << i << "  " << i << "     " << yd(i)
	<< "    # YD_{" << i << i << "}(Q)MSSM DRbar" << endl;
  }
  
  out << "Block yeHigh Q= " << displayMu() 
      << "   # diagonal lepton Yukawa matrix\n";
  for (i=1; i<=3; i++) {
    out << "  " << i << "  " << i << "     " << ye(i)
	<< "    # YE_{" << i << i << "}(Q)MSSM DRbar" << endl;
...
void FlavourMssmSoftsusy::msoftSLHAHigh(ostream & out) {
    /// Find the Yukawa rotation matrices such that the super-CKM basis may be
    /// defined
    DoubleMatrix Ud(3, 3), Vd(3, 3), Uu(3, 3), Vu(3, 3);
    DoubleVector yu(3), yd(3);
    ...
    out << "Block msq2High Q= " << displayMu() << " # super CKM squark mass^2 matrix"
	<< " - DRbar" << endl;
    int i, j; for (i=1; i<=3; i++)
      for (j=1; j<=3; j++) {
	out << "  " << i << "  " << j << "    "; 
    ...
    out << "Block msl2High Q= " << displayMu() << " # super MNS slepton mass^2 matrix"
	<< " - DRbar" << endl;
    for (i=1; i<=3; i++)
      for (j=1; j<=3; j++) {
	out << "  " << i << "  " << j << "    "; 
    ...
    out << "Block msd2High Q= " << displayMu() << " # super CKM squark mass^2 matrix"
	<< " - DRbar" << endl;
    for (i=1; i<=3; i++)
      for (j=1; j<=3; j++) {
	out << "  " << i << "  " << j << "    "; 
    ...
    out << "Block msu2High Q= " << displayMu() << " # super CKM squark mass^2 matrix"
	<< " - DRbar" << endl;
    for (i=1; i<=3; i++)
      for (j=1; j<=3; j++) {
	out << "  " << i << "  " << j << "    "; 
    ...
    out << "Block mse2High Q= " << displayMu() << " # super MNS slepton mass^2 matrix"
	<< " - DRbar" << endl;
    for (i=1; i<=3; i++)
      for (j=1; j<=3; j++) {
	out << "  " << i << "  " << j << "    "; 
    ...
    out << "Block tuHigh Q= " << displayMu() << "   # super CKM trilinear matrix"
	<< " - DRbar" << endl;
    for (i=1; i<=3; i++)
      for (j=1; j<=3; j++) {
	out << "  " << i << "  " << j << "    "; 
    ...
    out << "Block tdHigh Q= " << displayMu() << "   # super CKM trilinear matrix"
	<< " - DRbar" << endl;
    for (i=1; i<=3; i++)
      for (j=1; j<=3; j++) {
	out << "  " << i << "  " << j << "    "; 
    ...
    out << "Block teHigh Q= " << displayMu() << "   # super CKM trilinear matrix"
	<< " - DRbar" << endl;
    for (i=1; i<=3; i++)
      for (j=1; j<=3; j++) {
	out << "  " << i << "  " << j << "    "; 
  ...
  out << "Block VCKMHigh Q= " << displayMu() << " # DRbar CKM mixing matrix\n";
  for (i=1; i<=3; i++)
    for (j=1; j<=3; j++) {
      out << "  " << i << "  " << j << "    "; printRow(out, vCkm(i, j));
      out << "    # CKM_{" << i << j << "} matrix element" << endl;
    ...
    out << "Block msoftHigh Q= " << displayMu() 
	 << " # MSSM DRbar SUSY breaking parameters\n"; 
...
void FlavourMssmSoftsusy::lesHouchesAccordOutput(ostream & out, 
  ...
  if (!displayProblem().testSeriousProblem() || printRuledOutSpectra) {
    ...
    int n = 0; while (n < numPoints) {
      ...
      qMax = pars(1);
      drbarSLHAHigh(out, numPoints, qMax, n);
    }
  } else {
...
void FlavourMssmSoftsusy::minparSLHA(ostream & out, const char model [], 
  ...
  if (!strcmp(model, "sugra")) {
    out << "     1   "; printRow(out, sqrt(pars.display(1))); out << "   # m0" << endl;
  ...
  else if (!strcmp(model,"FGMCaseBNonRN2")){
    out << "     1   "; printRow(out, pars.display(1)); out << "   # lambda" << endl;
    out << "     2   "; printRow(out, pars.display(2)); out << "   # M_mess" << endl;
    out << "     4   "; printRow(out, pars.display(3)); out << "   # beta2" << endl;
    out << "     5   "; printRow(out, pars.display(4)); out << "   # betaeps" << endl;
    out << "     6   "; printRow(out, pars.display(5)); out << "   # thetavev" << endl;
    out << "     7   "; printRow(out, pars.display(6)); out << "   # cgrav" << endl;
  }
  else if (!strcmp(model,"FGMCaseBNonRN3")){
    out << "     1   "; printRow(out, pars.display(1)); out << "   # lambda" << endl;
    out << "     2   "; printRow(out, pars.display(2)); out << "   # M_mess" << endl;
    out << "     4   "; printRow(out, pars.display(3)); out << "   # beta2" << endl;
    out << "     5   "; printRow(out, pars.display(4)); out << "   # beta3" << endl;
    out << "     6   "; printRow(out, pars.display(5)); out << "   # betaeps" << endl;
    out << "     7   "; printRow(out, pars.display(6)); out << "   # thetavev" << endl;
    out << "     8   "; printRow(out, pars.display(7)); out << "   # cgrav" << endl;
  }
